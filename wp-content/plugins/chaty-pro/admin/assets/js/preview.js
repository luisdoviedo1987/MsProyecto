        ...
        if(activeChannels) {
            ...
            if(previewChannelList.length == 1 && jQuery("input[name='cta_type']:checked").val() != "chat-view") {
                ...
                for(i=0; i<previewChannelList.length; i++) {
                    ...
                    if(jQuery("input[name='cta_type']:checked").val() == "chat-view") {
                        console.log(jQuery(".csaas-chat-view-0 .csaas-channel-list").length);
                ...
                var closeHtml = '<div class="csaas-channel csaas-cta-close csaas-tooltip '+toolTipPosition+'" data-hover="'+jQuery("input[name='cht_close_button_text']").val()+'">' +
                    ...
                    '<svg viewBox="0 0 52 52" fill="none" xmlns="http://www.w3.org/2000/svg"><ellipse cx="26" cy="26" rx="26" ry="26" fill="'+ jQuery("input[name='cht_color']").val() +'"></ellipse><rect width="27.1433" height="3.89857" rx="1.94928" transform="translate(18.35 15.6599) scale(0.998038 1.00196) rotate(45)" fill="white"></rect><rect width="27.1433" height="3.89857" rx="1.94928" transform="translate(37.5056 18.422) scale(0.998038 1.00196) rotate(135)" fill="white"></rect></svg>' +
                    '</span>' +
                    '</button>' +
                    '</div>' +
                    '</div>';
...
function getCTAWidgetIcon() {
    ...
    if(widgetIcon == "chat-image") {
        if(jQuery("#elPreviewImage img").length && !isEmpty(jQuery("#elPreviewImage img").attr("src"))) {
            return "<span class='csaas-svg' style='background: "+jQuery("input[name='cht_color']").val()+"'><img id='cta-image' src='"+jQuery("#elPreviewImage img").attr("src")+"' alt='Chaty Widget' /></span>";
        }
        widgetIcon = 'chat-base';
    }

    return '<span class="csaas-svg">'+getSvgIcon(widgetIcon, jQuery("input[name='cht_color']").val())+"</span>";
}
