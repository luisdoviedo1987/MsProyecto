<?php
/**
 * weLaunch Framework Private Extended Functions Container Class
 *
 * @class       weLaunch_Functions_Ex
 * @since       3.0.0
...
// Don't duplicate me!
if ( ! class_exists( 'weLaunch_Functions_Ex', false ) ) {
	...
	class weLaunch_Functions_Ex {
		...
		public static function record_caller( $opt_name = '' ) {
			...
			if ( ! ( 'tools.php' === $pagenow && isset( $_GET['page'] ) && ( 'welaunch-framework' === $_GET['page'] || 'health-check' === $_GET['page'] ) ) ) {
				return;
			}
			...
			if ( ! empty( $caller ) && ! empty( $opt_name ) && class_exists( 'weLaunch_Core' ) ) {
				if ( ! isset( weLaunch_Core::$callers[ $opt_name ] ) ) {
					weLaunch_Core::$callers[ $opt_name ] = array();
				}

				if ( strpos( $caller, 'class-welaunch-' ) !== false || strpos( $caller, 'welaunch-core/framework.php' ) ) {
					return;
				}

				if ( ! in_array( $caller, weLaunch_Core::$callers[ $opt_name ], true ) ) {
					weLaunch_Core::$callers[ $opt_name ][] = $caller;
				}
		...
		public static function s() {
			if ( ! empty( get_option( 'welaunch_p' . 'ro_lic' . 'ense_key', false ) ) ) { // phpcs:ignore Generic.Strings.UnnecessaryStringConcat.Found
				$s = get_option( 'welaunch_p' . 'ro_l' . 'icense_status', false ); // phpcs:ignore Generic.Strings.UnnecessaryStringConcat.Found
		...
		/**
		 * Is weLaunch embedded inside a plugin.
		 *
		 * @param     string $file File to check.
		 *
		 * @return array|bool
		...
		/**
		 * Is weLaunch embedded in a theme.
		 *
		 * @param     string $file File to check.
		 *
		 * @return array|bool
		...
		public static function extension_compatibility( $parent, $path, $ext_class, $new_class_name, $name ) {
			...
			$upload_dir = weLaunchFramework::$_upload_dir . '/extension_compatibility/';
			if ( ! file_exists( $upload_dir . $ext_class . '.php' ) ) {
				...
				if ( ! file_exists( $upload_dir . $new_class_name . '.php' ) ) {
					$class_file = '<?php' . PHP_EOL . PHP_EOL .
						'class {{ext_class}} extends weLaunch_Extension_Abstract {' . PHP_EOL .
						'    private $c;' . PHP_EOL .
						...
						'    }' . PHP_EOL .
						'    // fake "extends weLaunch_Extension_Abstract\" using magic function' . PHP_EOL .
		...
		/**
		 * Check if weLaunch is activated.
		 *
		 * @access public
		 * @since 4.0.0
		 */
		...
		/**
		 * Set weLaunch to activated.
		 *
		 * @access public
		 * @since 4.0.0
		 */
		public static function set_activated() {
			weLaunch_Core::$insights->optin();
		}

		/**
		 * Set weLaunch to deactivated.
		 *
		 * @access public
		 * @since 4.0.0
		 */
		public static function set_deactivated() {
			weLaunch_Core::$insights->optout();
		}
		...
		public static function register_class_path( $prefix = '', $path = '' ) {
			if ( ! class_exists( 'weLaunch_Autoloader' ) ) {
				require_once weLaunch_Path::get_path( '/inc/classes/class-welaunch-autoloader.php' );
			}
			$loader = new weLaunch_Autoloader( $prefix, $path );
